...
package gs {
	...
	public class TweenLite {
		...
		public var onUpdate:Function; //The function that should be triggered when the tween updates values (on each frame)
		public var onUpdateParams:Array; //An array containing the parameters that should be passed to the onUpdate function.
		...
		public function initTweenVals():void {
			...
			if (this.target is Array) {
				var endArray:Array = [];
				for (p in this.vars) { //First find an instance of an array in the this.vars to match up with. There should never be more than one.
				...
				for (p in this.vars) {
					if (p == "volume" && this.target is MovieClip) { //If we're trying to change the volume of a MovieClip, then set up a quasai proxy using an instance of a TweenLite to control the volume.
					...
					} else if (p.toLowerCase() == "mccolor" && this.target is DisplayObject) { //If we're trying to change the color of a DisplayObject, then set up a quasai proxy using an instance of a TweenLite to control the color.
